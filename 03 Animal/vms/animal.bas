5 OPTION TYPE=EXPLICIT
6 DECLARE INTEGER I, K, N, X, Y, Z, Z1
7 DECLARE STRING A$, B$, C$, Q$, T$, V$, X$
...
135 A$=EDIT$(A$,32)     ! to uppercase
140 IF A$="LIST" THEN 600
150 IF LEFT$(A$,1)="Y" THEN 160
151 INPUT "DO YOU WANT TO QUIT";A$
152 IF EDIT$(LEFT$(A$,1),32)="Y" THEN 999
153 GOTO 130
160 K=1
170 GOSUB 390
180 IF LEN(A$(K))=0 THEN 999
190 IF LEFT$(A$(K),2)="\Q" THEN 170
200 PRINT "IS IT A " + RIGHT$(A$(K),3);
210 INPUT A$
220 A$=EDIT$(A$,32)
...
260 PRINT V$;" FROM A ";RIGHT$(A$(K),3)
270 INPUT X$
280 PRINT "FOR A ";V$;" THE ANSWER WOULD BE "; \ INPUT A$
300 A$=EDIT$(LEFT$(A$,1),32) \ IF A$<>"Y" AND A$<>"N" THEN 280
310 IF A$="Y" THEN B$="N"
320 IF A$="N" THEN B$="Y"
330 Z1=VAL(A$(0))
340 A$(0)=STR$(Z1+2)
...
400 Q$=A$(K)
410 FOR Z = 3 UNTIL Z > LEN(Q$) OR MID$(Q$,Z,1)="\"
415 PRINT MID$(Q$,Z,1); \ NEXT Z
420 INPUT C$
430 C$=EDIT$(LEFT$(C$,1),32)
...
600 PRINT LF + "ANIMALS I ALREADY KNOW ARE:"
605 X=0
610 FOR I=1 TO 200
620 IF LEFT$(A$(I),2)<>"\A" THEN 650
624 PRINT TAB(15*X);
630 FOR Z = 3 UNTIL Z > LEN(A$(I)) OR MID$(A$(I),Z,1)="\"
640 PRINT MID$(A$(I),Z,1); \ NEXT Z
